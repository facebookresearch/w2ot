...
from w2ot.external.jaxopt_backtracking_linesearch import BacktrackingLineSearch
  ...
  def update(self,
    ...
    if not isinstance(self.stepsize, Callable) and self.stepsize <= 0:
      ...
      if self.linesearch == "backtracking":
        ...
        init_stepsize = self.max_stepsize
        new_stepsize, ls_state = ls.run(init_stepsize,
                                        params, value, grad,
                                        descent_direction,
                                        *args, **kwargs)
        # jax.debug.print('a_k={}', new_stepsize)
